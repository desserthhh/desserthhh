   ...
   <!------------End Header ------------>
     ...
     <div class="content">
     	<div class="content_top">
    		...
    		<p><a href="/MStores/sale.action">售票</a> &gt;&gt;&gt; <a href="#" class="active">放映计划</a></p>
    	    </div>
    		<div class="clear"></div>
    	</div>
    <div class="component">
		...
		<table>
			...
			if(movie_name != null){
			   String[] question = (String[])request.getServletContext().getAttribute("question");
			...
			<% 
			   for(int i = 0;i<num;i++){
			%>
				<tr><td><%=movie_name[i] %></td><td><%=question[i] %></td><td><%=choices[i] %></td><td><%=pid[i] %></td><td><%=status[i] %></td></tr>
			<%
			   }
			}
			%>
    ...
    <div class="section group">
		<h1>添加活动</h1>
			<form method="post" action="/MStores/setactivity">
                            相关电影：<input type="text" name="mid" value=""/>
                            问题：<input type="text" name="question" value=""/>
                            问题选项：<input type="text" name="choices" value=""/>
                            匹配放映计划：<input type="text" name="pid" value=""/>
